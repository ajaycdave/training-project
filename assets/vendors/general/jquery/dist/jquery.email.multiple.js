    ...
    $.fn.email_multiple = function(options) {
        ...
        return this.each(function()
        {
            ...
            $element.keydown(function (e) {
                ...
                if (e.keyCode === 13 || e.keyCode === 32) {
                    ...
                    if (/^[a-z0-9._-]+@[a-z0-9._-]+\.[a-z]{2,6}$/.test(getValue)){
                        $('.all-mail').append('<span class="email-ids"><font>' + getValue + '</font><span class="cancel-email">x</span></span>');
                        $element.val('');
            ...
            $(document).on('click','.cancel-email',function(){
                    var remove_email=$(this).parent().children().html();
                    $(this).parent().remove();
                    data.splice( $.inArray(remove_email,data),1);
                        email=data.join(";");
                    //var d=data.join(";");
                     $('#straight_forward_email').val(email);
            ...
            if(settings.data){
                $.each(settings.data, function (x, y) {
                    if (/^[a-z0-9._-]+@[a-z0-9._-]+\.[a-z]{2,6}$/.test(y)){
                        $('.all-mail').append('<span class="email-ids"><font>' + y + '</font><span class="cancel-email">x</span></span>');
                        $element.val('');
